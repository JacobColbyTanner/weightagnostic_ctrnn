...
def fitnessFunction_vehicle_robust(ctrnn_parameters, ctrnn_size, ctrnn_step_size, duration, distance, bv_step_size, transient_steps, robust_range, discrete=True):
    
    range_robust = np.arange(robust_range[0],robust_range[1],1)
    fitness_all = np.zeros(len(range_robust))
    for multiplier in range_robust:
        ...
        fitness_all[multiplier-1] = np.clip(1 - ((finaldistance / steps) / distance), 0, 1) - 0.01*np.sum(np.abs(ctrnn.weights))/(ctrnn.size**2)

    fitness = np.mean(fitness_all)
    return fitness
